...
import contactImage from "../image/Dev-stu.jpg";

function H_dev_stu({ departments, contactInfo }) {
  return (
    <div>
      {/* Header Section */}
      <header
          ...
          <ul className="space-y-8 font-sf">
            {departments.map(({ name, path }) => (
              <li key={path}>
                <Link
                  to={path}
                  className="flex justify-between items-center bg-chemjruk px-4 py-2 w-full max-w-[800px] rounded-md shadow-md hover:bg-red-800 transition-colors duration-200"
                >
